...
export const ApiKey = {
  ...
  async create({ key, email, duration, expiresAt, isActive = true, usageLimit = 100 }) {
    try {
      ...
      if (isMissingColError(e)) {
        console.warn('[ApiKey.create] missing col, migrating then fallback');
        await ensureCols();
        // coba lagi dengan COLS setelah migrasi
        try {
          const { rows } = await query(
            `INSERT INTO api_keys (key, email, duration, expires_at, is_active, usage_limit, usage_count)
             VALUES ($1, $2, $3, $4, $5, $6, 0)
          ...
          if (isMissingColError(e2)) {
            // fallback legacy: insert tanpa usage_* agar addkey tetap sukses di DB lama
            console.warn('[ApiKey.create] fallback legacy insert tanpa usage_limit');
            const { rows } = await query(
            ...
            return rows[0] ? { ...rows[0], usageLimit, usageCount: 0, lastHitAt: null, updatedAt: null } : null;
          }
          throw e2;
        }
      }
  ...
  async deactivate(key) {
    try {
      ...
      if (isMissingColError(e)) {
        await ensureCols();
        try { await query(`UPDATE api_keys SET is_active = false, updated_at = now() WHERE key = $1`, [key]); }
        catch { await query(`UPDATE api_keys SET is_active = false WHERE key = $1`, [key]); }
      } else throw e;
    }
  },
  async deleteExpired() {
  ...
  async consume(key) {
    ...
    try {
      ...
      if (row.expiresAt && new Date() > new Date(row.expiresAt)) {
        try { await client.query(`UPDATE api_keys SET is_active = false, updated_at = now() WHERE key = $1`, [key]); }
        catch { await client.query(`UPDATE api_keys SET is_active = false WHERE key = $1`, [key]); }
      ...
      if (row.lastHitAt) {
        ...
        if (delta < THROTTLE_MS) {
          ...
          return { allowed: false, status: 429, retryAfter, body: { error: 'Too many requests', message: `Jeda 5 detik per hit. Coba lagi dalam ${retryAfter} detik.`, retryAfter } };
        }
      }
      if (row.usageLimit !== null && row.usageLimit !== undefined) {
        if (row.usageCount >= row.usageLimit) {
          await client.query('ROLLBACK');
          return { allowed: false, status: 429, body: { error: 'Daily limit exceeded', message: `Limit harian ${row.usageLimit} tercapai. Reset otomatis jam 00:00 WIB.`, limit: row.usageLimit, used: row.usageCount, remaining: 0, resetAt: getResetAtWIB() } };
        }
      }
      let updated;
      try {
        const res = await client.query(`UPDATE api_keys SET usage_count = usage_count + 1, last_hit_at = now(), updated_at = now() WHERE key = $1 RETURNING ${COLS}`, [key]);
        updated = res.rows;
      } catch (e) {
        if (isMissingColError(e)) {
          await ensureCols();
